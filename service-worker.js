const CACHE_NAME = 'ps-cafe-v5-2-stable-rebuild-20260515-1';
const ASSETS = [
  './', './index.html', './manifest.json', './service-worker.js',
  './wallpaper.jpeg', './icon.jpeg', './icon-192.png', './icon-512.png'
];
...
self.addEventListener('activate', event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== CACHE_NAME && k.startsWith('ps-cafe')).map(k => caches.delete(k)))).then(() => self.clients.claim()));
});
